use crate::{
    utils::conversion::{PyToRust, RustToPy},
    PyAnnData,
};
...
impl PyChunkedMatrix {
    ...
    fn __next__<'py>(mut slf: PyRefMut<Self>, py: Python<'py>) -> Option<(PyObject, usize, usize)> {
        slf.0
            .next()
            .map(|(data, i, j)| (data.rust_into_py(py).unwrap(), i, j))
    }
}
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.current_row >= self.total_rows {
            ...
            Python::with_gil(|py| -> PyResult<_> {
                ...
                let data = self
                    .matrix
                    .call_method1("__getitem__", (range,))?
                    .into_rust(py)?;
                Ok(Some((data, i, j)))
            })
            .unwrap()
...
impl<'py> AnnDataIterator for PyAnnData<'py> {
    ...
    fn read_obsm_item_iter<'a>(
        ...
        Ok(PyMatrixIterator::new(
            self.getattr("obsm")?.call_method1("__getitem__", (key,))?,
            chunk_size,
        )?)
    }
        ...
        D: H5Type + Copy + Send + Sync + std::cmp::PartialEq + std::fmt::Debug,
    {
        ...
        self.set_n_obs(csr.nrows())?;
        self.getattr("obsm")?
            .call_method1("__setitem__", (key, csr.rust_into_py(self.py())?))?;
        Ok(())
    }
}
