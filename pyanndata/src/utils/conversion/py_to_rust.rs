use crate::utils::{conversion::dataframe::to_rust_df, instance::*};
...
use anndata_rs::data::{Data, Mapping, MatrixData, Scalar};
...
impl<T: numpy::Element> PyToRust<CsrMatrix<T>> for &PyAny {
    fn into_rust<'py>(self, _py: Python<'py>) -> PyResult<CsrMatrix<T>> {
        ...
        let mat = CsrMatrix::try_from_csr_data(
            shape[0],
            shape[1],
            indptr,
            indices,
...
impl PyToRust<Box<dyn Data>> for &PyAny {
    fn into_rust<'py>(self, py: Python<'py>) -> PyResult<Box<dyn Data>> {
        if isinstance_of_arr(py, self)? {
            ...
            let df: DataFrame = py
                .import("polars")?
                .call_method1("from_pandas", (self,))?
                .into_rust(py)?;
            Ok(Box::new(df))
        } else {
            panic!(
                "Cannot convert Python type \"{}\" to 'dyn Data'",
                self.get_type()
            )
        }
    }
...
impl PyToRust<Box<dyn MatrixData>> for &PyAny {
    fn into_rust<'py>(self, py: Python<'py>) -> PyResult<Box<dyn MatrixData>> {
        if isinstance_of_arr(py, self)? {
            ...
            panic!(
                "Cannot convert Python type \"{}\" to 'dyn MatrixData'",
                self.get_type()
            )
        }
    }
