    ...
    ($py:expr, $data:expr, $dtype:expr) => {
        match $dtype {
            Unsigned(IntSize::U1) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "uint8", ($data.downcast_ref::<Scalar<u8>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Unsigned(IntSize::U2) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "uint16", ($data.downcast_ref::<Scalar<u16>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Unsigned(IntSize::U4) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "uint32", ($data.downcast_ref::<Scalar<u32>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Unsigned(IntSize::U8) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "uint64", ($data.downcast_ref::<Scalar<u64>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Integer(IntSize::U1) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "int8", ($data.downcast_ref::<Scalar<i8>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Integer(IntSize::U2) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "int16", ($data.downcast_ref::<Scalar<i16>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Integer(IntSize::U4) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "int32", ($data.downcast_ref::<Scalar<i32>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Integer(IntSize::U8) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "int64", ($data.downcast_ref::<Scalar<i64>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Float(FloatSize::U4) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "float32", ($data.downcast_ref::<Scalar<f32>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Float(FloatSize::U8) => Ok(PyModule::import($py, "numpy")?.call_method1(
                "float64", ($data.downcast_ref::<Scalar<f64>>().unwrap().0.to_object($py),)
                )?.to_object($py)),
            Boolean => Ok($data.downcast_ref::<Scalar<bool>>().unwrap().0.to_object($py)),
